...
type SuidCheckboxProps = CheckboxTypeMap['props'] & CheckboxTypeMap['selfProps'];
...
export const Checkbox: Component<CheckboxProps> = (props) => {
  ...
  const onChange: CheckboxProps['onChange'] = (event, checked) => {
    ...
    local.formHandler?.setFieldValue?.(rest.name, getValue(checked), { validateOn: [event.type] });
  ...
  return (
    <div>
      <FormControlLabel
        control={<SuidCheckbox {...rest} checked={store.checked} id={store.id} onChange={onChange} onBlur={onBlur} />}
        label={local.label}
      />
