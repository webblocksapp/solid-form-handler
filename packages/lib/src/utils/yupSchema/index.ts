...
import { FIELDSETS_KEY } from '@constants';
...
export const yupSchema = <T>(schema: SchemaOf<T>): ValidationSchema<T> => {
  ...
  const isFieldFromSchema = (path: string) => {
    ...
    /**
     * FIELDSETS_KEY is a reserved key autogenerated by the form handler
     * when the form data structure is an array.
     */
    if (path === FIELDSETS_KEY) return true;
  ...
  const validateAt: ValidationSchema<T>['validateAt'] = async (path, data, options) => {
    try {
      if (path === FIELDSETS_KEY) await schema.validate(data, { abortEarly: false, recursive: false });
      if (path !== FIELDSETS_KEY) await schema.validateAt(path, data, options);
    } catch (error) {
