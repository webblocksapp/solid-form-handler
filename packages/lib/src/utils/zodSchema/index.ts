...
export const zodSchema = <T>(schema: ZodSchema<T>): ValidationSchema<T> => {
  ...
  const isFieldFromSchema = (path: string) => {
     ...
     * for storing the state of the whole form data.
     */
    if (path === ROOT_KEY) return true;
    ...
    if (path === ROOT_KEY) {
      try {
        ...
        if (error instanceof ZodError) {
          ...
          throw new ValidationError(path, DATA_CONTAINS_ERRORS, children);
        }
      }
    ...
    if (store.effects === undefined) {
      ...
      try {
        ...
        if (error instanceof ZodError) {
          ...
          message = message || error.errors.map((item) => item.message).join(', ');
  ...
  const getFieldDataType = (path: string) => {
     ...
     *
     * If path equals to ROOT_KEY, the datatype is retrieved from the
     * whole schema.
     */
