...
export const Checkbox: Component = () => (
  <DocsContentLayout>
    ...
    <p>
      ...
      redefining all the props of a vanilla HTML checkbox (open closed
      principle). If you will use a <i>SolidJS</i> UI library that provides a
      predefined <code>Checkbox</code> component, the same approach can be done
      by extending the <code>CheckboxProps</code> from the{' '}
      <code>UILibraryCheckboxProps</code> interface.
    </p>
    <p>
      At <code>CheckboxProps</code> the <code>formHandler</code> is defined as
      an optional prop to preserve the original nature of a checkbox (open
      closed principle) which is not dependent on a <code>formHandler</code>{' '}
      prop. The <code>onChange</code> event was extended by the method{' '}
